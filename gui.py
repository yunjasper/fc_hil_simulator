...
layout_sim_config_options = [
    ...
    # radio buttons
    [sg.Text('Target DUT: ', size=(15, 1)), sg.Radio('Hardware', 'target', size=(12, 1), default=True, key='radio_target_hw'), sg.Radio('Software', 'target', size=(12, 1), key='radio_target_sw')],
...
layout_rocket_options = [
    ...
    [sg.Text('Rocket mass (kg): ', size=rkt_options_text_size), sg.Input(focus=True, key='input_rkt_mass', default_text=str(utils.Settings.RKT_MASS_KG), size=rkt_options_input_size)],
    [sg.Text('Drogue parachute Cd: ', size=rkt_options_text_size), sg.Input(focus=True, key='input_drogue_cd', default_text=str(utils.Settings.RKT_DROGUE_DRAG_COEFF), size=rkt_options_input_size)],
    [sg.Text('Drogue parachute area (m2): ', size=rkt_options_text_size), sg.Input(focus=True, key='input_drogue_area', default_text=str(utils.Settings.RKT_DROGUE_AREA), size=rkt_options_input_size)],
    [sg.Text('Main parachute Cd: ', size=rkt_options_text_size), sg.Input(focus=True, key='input_main_cd', default_text=str(utils.Settings.RKT_MAIN_DRAG_COEFF), size=rkt_options_input_size)],
    [sg.Text('Main parachute area (m2): ', size=rkt_options_text_size), sg.Input(focus=True, key='input_main_area', default_text=str(utils.Settings.RKT_MAIN_AREA), size=rkt_options_input_size)],
    [sg.Text('Launch rail angle (deg): ', size=rkt_options_text_size), sg.Input(focus=True, key='input_launch_angle', default_text=str(utils.Settings.RKT_LAUNCH_ANGLE), size=rkt_options_input_size)],
...
layout_sim_results = [
    ...
    [sg.Text('Simulator state: ', size=sim_results_label_size), sg.Text('MAIN_DESCENT', size=sim_results_data_size, key='text_sim_state'), 
    ...
    [sg.Text('Target state: ', size=sim_results_label_size), sg.Text('PAD', size=sim_results_data_size, key='text_target_state'), 
     sg.Text('X velocity (m): ', size=sim_results_label_size), sg.Text('0', size=sim_results_data_size, key='text_x_vel'),
     sg.Text('Z velocity (m): ', size=sim_results_label_size), sg.Text('0', size=sim_results_data_size, key='text_z_vel')],
    [sg.Text('Simulation time (ms): ', size=sim_results_label_size), sg.Text('0', size=sim_results_data_size, key='text_sim_time_ms'), 
     sg.Text('X acceleration (m): ', size=sim_results_label_size), sg.Text('0', size=sim_results_data_size, key='text_x_acc'),
     sg.Text('Z acceleration (m): ', size=sim_results_label_size), sg.Text('0', size=sim_results_data_size, key='text_z_acc')],
     # plot window
...
# top-level layout
layout = [
    [sg.TabGroup([[sg.Tab('Configuration', layout_sim_config_options), sg.Tab('Rocket', layout_rocket_options), sg.Tab('Results', layout_sim_results)]])]
]
...
def update_sim_config(values, window):
    if values is None:
        # restore to defaults
        window['radio_target_hw'].update(True)
        window['radio_target_sw'].update(False)
...
window = sg.Window('MRT FC HIL Simulator', layout, font=("Helvetica", 12), resizable=True)
